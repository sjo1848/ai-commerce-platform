...
Read `AGENTS.md`, `.orchestration/RUNTIME-POLICY.md`, `.opencode/MODEL-ROUTING.md`, the active contract/block plan/invariants, then verify the exact candidate HEAD and working-tree state before accepting any evidence.
...
Do not redesign the system from QA. If a systemic issue appears, return `ROOT_CAUSE_REVIEW_REQUIRED` with evidence.
