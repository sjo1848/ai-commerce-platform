...
test("R2.8.4 LLM language corpus has bounded structured outcomes", () => {
  ...
  assert.match(llmSource, /Quantities explicitly attached to personas/); assert.match(llmSource, /For 'X en vez de Y'/); assert.match(llmSource, /La 101, en vez de la 102/); assert.match(llmSource, /Do not add room 103 or any other available room/); assert.match(llmSource, /Quiero la habitación 999/); assert.match(llmSource, /not among the current authoritative candidates/);
