...
export type UserSemanticPatch = {
  ...
  stay?: {
    checkIn?: ExplicitPatch<string>;
    checkOut?: ExplicitPatch<string>;
    guests?: ExplicitPatch<number>;
  };
...
export type InvocationAuthorityRef = {
  kind: "invocation";
  invocationId: string;
  dependencyFingerprint: string;
};

export type OperationAuthorityRef = {
  kind: "operation";
  operationId: string;
...
export type ToolObservationPayload =
  ...
  | {
      ...
      authority: OperationAuthorityRef;
...
export type ServerControlPayload =
  ...
  | {
      ...
      operationId: string;
      operationFingerprint: string;
      status: "approval_required" | "approved" | "invalidated";
    }
...
export type TaskEventEnvelope<K extends string, P> = {
  eventId: string;
  kind: K;
  sessionId: string;
  taskId: string;
...
export type TaskStateInvalidation = {
  target: InvalidationTarget;
  reason: "dependency_changed" | "observation_replaced";
  causedBy: readonly string[];
};

export type TaskStateReduction = {
  state: import("./contracts.js").TaskState;
  accepted: boolean;
  material: boolean;
  duplicate: boolean;
  invalidations: readonly TaskStateInvalidation[];
