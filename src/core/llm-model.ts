...
export class LLMModelRouter implements ModelRouter {
  ...
  private async repairContradictoryToolRoute(
    ...
    context: ExecutionContext,
    availableTools: readonly ToolDescriptor[],
    onProviderFailure?: (failureCategory: string | undefined) => void,
  ...
  async route(
    ...
    try {
      ...
      if (value.kind !== "tool" || typeof value.toolId !== "string" || !isRecord(value.input) || clarification.reason !== "none" || clarification.missing.length !== 0) {
        ...
        const repaired = await this.repairContradictoryToolRoute(
          value,
          system,
          message,
          context,
          availableTools,
          (category) => { repairFailureCategory = category; },
        );
        if (repaired) return repaired;
        return this.fallbackRoute(
          "invalid_tool_plan_shape",
          message,
          context,
          availableTools,
          conversation,
