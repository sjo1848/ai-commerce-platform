...
- The exact execution SHA is captured in the evidence produced by the actual gate/run; it is not fixed in this contract.
- R2.8.4: `NOT CLOSED`.
- R2.8.5: `BLOCKED`.
...
- Stop on first contractual failure and retain the partial report; do not send subsequent validation requests to accumulate failures. Missing required direct evidence or captured invalid evidence fails admission; missing supplemental telemetry remains `UNKNOWN_NOT_CAPTURED` and does not fail when direct authoritative evidence is sufficient.
...
- Correlate requestId, sessionId, experimentId, trace, exact Worker Version and candidate SHA where captured. Missing route telemetry, historical query results, or live-tail transport remains `UNKNOWN_NOT_CAPTURED`/supplemental and is not inferred as zero; when direct authoritative evidence is sufficient, that missing supplemental telemetry does not fail the run. Captured invalid/negative evidence (including fallback or wrong-version evidence) remains fail-closed. Stored positive durable-budget reconciliation is the specific material exception and is required before RUN 1 is declared reconciled GREEN; a request-ID-only query is insufficient. Missing, UNKNOWN, mismatched, invalid or unreconciled durable-budget evidence must classify as `BUDGET_RECONCILIATION_NOT_PROVEN` and fail closed.
